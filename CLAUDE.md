...
## Screenshot Workflow
- Puppeteer is installed locally in `node_modules/` (run `npm install puppeteer` if missing). Its Chrome for Testing build is cached at `~/.cache/puppeteer/`. If the browser is missing, run `npx puppeteer browsers install chrome`.
